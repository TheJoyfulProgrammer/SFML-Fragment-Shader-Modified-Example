...
vec3 grain1(vec3 v, float n){
	...
	l = length(v.xy); 
	l = pow(l, 2.0); 
	l += octaves(v  * n *  vec3(0.014, 0.014, 0.0042), 2.3) * 1.6; 
	w =cos(2.0 * PI * l * 8.0); 
	w = sigmoid1(w + 0.85, 3.0); // -1.0, 1.0
	r = snoise(v * vec3(160.0, 160.0, 6.5)) + 0.5; 
	float p = 0.6;
	m = w * p + r * (1.0 - p); 
	m = (m + 1.0) / 2.0; // 0.0, 1.0
	...
	col += octaves(v * vec3(1.0, 1.0, 0.2), 2.3) * 0.05; 
	col = clamp(col, 0.0, 1.0); // 0.0, 1.0
	return col;
}
...
vec3 grain2(vec2 v){
	...
	float val = abs(sin(time*0.001)) * 0.027;
	
	return grain1(pos, 3.0 + val);
}

vec3 tile(vec2 uv, vec2 div){
	vec2 req_uv = fract(uv * div); 
	float index = 
		floor(uv.x * div.x) + 
...
void main( void ) {
	vec2 p = (( gl_FragCoord.xy / resolution.y ) - vec2(0.5 * resolution.x / resolution.y, 0.5)) * mouse.x - vec2(0.05, 0.05);
	vec2 div1 = vec2(4.0);
	vec2 div2 = vec2(1.0, ceil(10.0 * mouse.y));
