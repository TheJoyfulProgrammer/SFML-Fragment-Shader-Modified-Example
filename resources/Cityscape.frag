...
void main( void ) 
{
	...
	// palm-tree
	float r = .2 + 0.1*cos( atan(p.y,p.x)*10.0 + 30.0*p.x + 1.5 * sin(time));	
	color *= smoothstep( r, r+0.01, length( p ) );

	r = 0.015;
	r += 0.002*sin(120.0*uv.y);
	r += exp(-40.0*uv.y);
 	color *= 1.0 - (1.0-smoothstep(r,r+0.002, abs(p.x-.25*sin(2.0*p.y))))*(1.0-smoothstep(0.0,0.1,p.y));
	...
	// cityline
	if(-p.y< N(ceil(p.x*20.)).x*.5) final-=final*p.y;
