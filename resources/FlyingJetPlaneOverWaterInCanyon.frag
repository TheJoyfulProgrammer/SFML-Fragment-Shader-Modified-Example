/*
 * Original shader from: https://www.shadertoy.com/view/DdcXDX
 */
...
mat2 rotyz=mat2(0.), rotxy=mat2(0.), rotxz=mat2(0.);
...
float fbm1(in vec2 uv,int octaves,float i1,float value1)
{
  ...
  //  float f = 2.15+(sin(uv.x/i1)+cos(uv.x/i1))*value1;
    ...
    float n1 = 0.;        vec2 noise2 = vec2(0.),noise1 = vec2(0.);
    for (int i = 0; i < 10; i++)
    {
        if (i >= octaves) break;
...
vec3 normal(vec3 pos)
{
    ...
    return normalize( sign(e.yxx)*de( pos + e.yxx ) +
                      sign(e.xyx)*de( pos + e.xyx ) +
                      sign(e.xxy)*de( pos + e.yxy ) +
                      sign(e.xxx)*de( pos + e.xxx ) );
}
...
vec3 march(vec3 from, vec3 dir, vec2 uv)
{
    ...
    float d=0., td=0., td2=0., vid=0., g=0.,aro=g, chorro=1.;
