/*
 * Original shader from: https://www.shadertoy.com/view/7sGBRD
 */
...
#define march_macro(func, ro, rd, steps, tmin, tmax, res) { float t = tmin; for (int i = 0;i<100; ++i) { vec3 p = (ro) + (rd) * t; float d = func(p); if (abs(d) < 0.001) { res = vec2(t, 1.); break; } t += d; if (t > (tmax) || i > (steps)) {res = vec2(t, -1.);break;}}}
#define get_norm_macro(func, p, res) { mat3 k = mat3(p, p, p) - mat3(0.0001); res = normalize(vec3(func(p)) - vec3(func(k[0]), func(k[1]), func(k[2]))); }
