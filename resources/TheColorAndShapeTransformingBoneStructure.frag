/*
 * Original shader from: https://www.shadertoy.com/view/sdGyDW
 */
...
float Voronoi(vec2 p, float M, float T, float l)  // --- Voronoi
{
    ...
    float m = 8., md = m, d, f;
    for (int i=0; i<9; i++)
        c = vec2(int_mod(i,3),i/3)-1.,
        h = hexPt(pH + c, T, l) - p,
        d = dot(h, h),
        d < md ? md = d, mo = h, C = c : C;

    for (int i=0; i<9; i++)
        h = hexPt(pH + vec2(int_mod(i,3),i/3)-1. + C, T, l) - p - mo,
...
void mainImage( out vec4 O, in vec2 g )
{
    ...
    const float LAYERS =11.;        // num of layers
