/*
 * Original shader from: https://www.shadertoy.com/view/sdKcDz
 */
...
float vesica(vec2 p, vec2 sz) {
  ...
  vec2 sz2 = sz*sz;
  float d  = (sz2.x-sz2.y)/(2.0987*sz.y);
...
vec3 smear(vec2 p, vec2 q) {
  ...
  const vec3 col11 = HSV2RGB(vec3(0.9, 0.9, 0.598));
  const vec3 col21 = HSV2RGB(vec3(0.4, 0.9, 0.50987));
